...
def fetch_data(github_id, github_password):
    ...
    fetch_dates = list(reversed([(yesterday - datetime.timedelta(i)).strftime("%Y_%m_%d") for i in range(DATA_LENGTH)]))
    data = {}
    ...
    for theme in themes:
        ...
        for date in fetch_dates:
            file_path = os.path.join(REPOSITORY_DATA_PATH, theme.name, date + ".json")
            try:
...
def generate_theme_pages(data):
    ...
    for theme in data.keys():
        ...
        generated_text = template.render({"data": json.dumps(data[theme]), "theme": theme})
        with open(result_path, "w", encoding="utf-8") as f:
            f.write(generated_text)
